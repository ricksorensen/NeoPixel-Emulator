...
class Adafruit_NeoMatrix(Adafruit_GFX):
    ...
    def create_matrix(self, width, height, pin, matrix_type, pixsize=35):
        self.width = width
        self.height = height
...
if __name__ == "__main__":
    ...
    matrix.create_matrix(
        ...
        10,
        6,
        matrix.positions["NEO_MATRIX_TOP"]
        + matrix.positions["NEO_MATRIX_LEFT"]
        + matrix.positions["NEO_MATRIX_COLUMNS"]
        + matrix.positions["NEO_MATRIX_PROGRESSIVE"],
        pixsize=10,
    )
    matrix.begin()
    matrix.show()
