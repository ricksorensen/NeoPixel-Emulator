...
class effect_panel:
    ...
    def firelight(
        ...
        pixstart = max(0, self._npix - firelen) if top else 0
        ...
        for led in range(pixstart, pixend):
            ...
            self._pix[led] = (R, G, B)
            if led < pixstart or led >= pixend:
                print(f"led out of range: {led}")
        if debug:
            ...
            try:
                ...
                for block in range(blocks * 3 // 4):
                    try:
                        ...
                        if start_led < pixstart or end_led > pixend:
                            print(f"sled={start_led} eled={end_led} blk={block}")
                        ...
                        for led in range(start_led, end_led):
                            self._pix[led] = (R, G, B)
                            if led < pixstart or led >= pixend:
                                print(f"new led out of range: {led}")
